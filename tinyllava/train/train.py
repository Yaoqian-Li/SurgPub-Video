...
def make_conversation_video(example):
    ...
    # pass-through fields used by trainer/reward
    if "solution" in example:
        result["solution"] = example["solution"]
    if "video_filename" in example:
        result["video_filename"] = example["video_filename"]
    if "frames" in example:
        result["frames"] = example["frames"]
    if "video" in example:
        result["video"] = example["video"]
    return result
