...
class LLaVATrainer_Reason(Trainer):
    ...
    def compute_loss(self, model, inputs, num_items_in_batch=None):
        ...
        for (cur_idx, cur_input) in enumerate(inputs):
            videos = []
            # 1) Prefer frames if provided (coldstart-style)
            if "frames" in inputs[cur_idx] and inputs[cur_idx]["frames"] is not None:
                frame_files = inputs[cur_idx]["frames"]
                for frame_path in frame_files:
                    try:
                        if isinstance(frame_path, str) and os.path.isabs(frame_path):
                            img = Image.open(frame_path).convert('RGB')
                        else:
                            img = Image.open(frame_path).convert('RGB')
                        img_tensor = self.video_preprocess(img)
                        videos.append(img_tensor)
                    except Exception:
                        continue
                if len(videos) == 0:
                    # fallback to video file if frames failed
                    if "video_filename" in inputs[cur_idx]:
                        video_file = inputs[cur_idx]["video_filename"]
                        if isinstance(video_file, str) and video_file.startswith('/'):
                            full_video_path = video_file
                        else:
                            full_video_path = self.data_path + video_file[1:] if isinstance(video_file, str) and len(video_file) > 0 and video_file[0] == '/' else os.path.join(self.data_path, video_file)
                        video = EncodedVideo.from_path(full_video_path, decoder="decord", decode_audio=False)
                        ...
                        for frame in video_data:
                            videos.append(self.video_preprocess(frame))
                else:
                    # 如果帧数过多，下采样到固定帧数
                    if len(videos) > self.num_frame:
                        indices = np.linspace(0, len(videos) - 1, self.num_frame, dtype=int)
                        videos = [videos[i] for i in indices]
            # 2) Else use video file by video_filename
            elif "video_filename" in inputs[cur_idx]:
                video_file = inputs[cur_idx]["video_filename"]
                if isinstance(video_file, str) and video_file.startswith('/'):
                    full_video_path = video_file
                else:
                ...
                # no frames or video, provide zeros as placeholder (unlikely but safe)
                videos = []

            if len(videos) == 0:
                # fallback placeholder to avoid crash
                crop_size = getattr(self.video_preprocess.image_processor, 'crop_size', getattr(self.video_preprocess.image_processor, 'size'))
                h = crop_size['height'] if 'height' in crop_size else crop_size
                w = crop_size['width'] if 'width' in crop_size else crop_size
                videos = [torch.zeros(3, h, w) for _ in range(self.num_frame)]
