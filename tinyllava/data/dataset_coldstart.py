    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            try:
                ...
                print('wrong path',image_folder, image_file)
            image = self.image_preprocess(image)
            data_dict['image'] = image
        elif 'frames' in sources:
            
            frame_files = self.list_data_dict[i]['frames']
            frame_folder = self.data_args.video_folder
            frames = []
            for frame_file in frame_files:

                frame = Image.open(os.path.join(frame_folder, frame_file)).convert('RGB')
